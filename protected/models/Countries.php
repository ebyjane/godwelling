 ...
 */
class Countries extends CiiModel
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('iso, name, nicename', 'required'),
			array('name, nicename', 'length', 'max'=>150),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, iso, name, nicename', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'parent' => array(self::BELONGS_TO, 'Countries', 'id'),
			'contents' => array(self::HAS_MANY, 'Content', 'country_id'),
		);
	}
	...
	{
		$this->name = $this->verifySlug($this->name, $this->nicename);
	...
	public function beforeSave()
	{
        ...
        Yii::app()->cache->delete('countries-listing');
		Yii::app()->cache->delete('countries');
		Yii::app()->cache->delete('WFF-countries-url-rules');
		Yii::app()->cache->delete('countries-pid');
	...
	public function getParentCategories($id)
	{
		...
		$response = Yii::app()->cache->get('countries-pid');
		if ($response == NULL)
		{
			$response = Yii::app()->db->createCommand('SELECT id, name, nicename FROM country')->queryAll();
			Yii::app()->cache->set('countries-pid', $response);
		}
		
		return $this->__getParentCountries($response, $id);
	}
	
	private function __getParentCountries($all_countries, $id, array $stack = array())
	{
		if ($id == 1)
		{
		...
		foreach ($all_countries as $k=>$v)
		{
			if ($v['id'] == $id)
			{
				$stack[$v['name']] = array(str_replace(Yii::app()->baseUrl, NULL, Yii::app()->createUrl($v['nicename'])));
				return $this->__getParentCountries($all_countries, $v['parent_id'], $stack);
			}
		}
	}
    ...
    public function checkSlug($slug, $id=NULL)
    {
        ...
        $count = $this->countByAttributes(array('nicename'=>$slug . $id));
